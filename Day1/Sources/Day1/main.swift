...
func totalFuelRequired() -> Int64 {
    let masses = readLinesAsInt64(path: "\(currentDir(currentFile: #file))/input-the-tyranny-of-the-rocket-equation.txt")
    return masses.map { fuelForMass(mass: $0) }.reduce(0, +)
}

func totalFuelWithAddedRequired() -> Int64 {
    let masses = readLinesAsInt64(path: "\(currentDir(currentFile: #file))/input-the-tyranny-of-the-rocket-equation-2.txt")
