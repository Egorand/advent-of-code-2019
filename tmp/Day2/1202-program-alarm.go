...
func main() {
	// Part 1
	program, err := readProgram("input-1202-program-alarm.txt")
	if err != nil {
		log.Fatal(err)
	}

	prepareProgram(program, 12, 2)
	executeProgram(program)
	fmt.Println(program)

	// Part 2
	program2, _ := readProgram("input-1202-program-alarm.txt")
	noun, verb := findNounAndVerb(program2, 19690720)
	fmt.Printf("[%d %d]\n", noun, verb)
}
...
func readProgram(path string) ([]int, error) {
	...
	for scanner.Scan() {
		...
		for _, instruction := range program {
			instructionCode, _ := strconv.Atoi(instruction)
			instructions = append(instructions, instructionCode)
		}
	}
	return instructions, scanner.Err()
}

func prepareProgram(program []int, noun int, verb int) {
	program[1] = noun
	program[2] = verb
}
...
func executeProgram(program []int) {
	...
	for {
		...
		case 1, 2:
			...
			resultIndex := program[pointer + 3]
			if opcode == 1 {
				program[resultIndex] = param1 + param2
			} else if opcode == 2 {
				program[resultIndex] = param1 * param2
			}
			pointer += 4
		case 99: return
		default: log.Fatal("Unknown opcode %d at position!", opcode, pointer)
		}
	}
}

func findNounAndVerb(program []int, target int) (int, int) {
	for noun := 0; noun <= 99; noun++ {
		for verb := 0; verb <= 99; verb++ {
			programCopy := program
			prepareProgram(programCopy, noun, verb)
			executeProgram(programCopy)
			if programCopy[0] == target {
				return noun, verb
			}
		}
	}
